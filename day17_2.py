    ...
    while len(queue) > 0:
        ...
        if dir == 'v':
            ...
            for dist in range(1,11):
                ...
                if j + dist < len(grid[0]):
                    sum += grid[i][j + dist]
                if dist >= 4 and j + dist < len(grid[0]) and v not in visited:
                    f = g + sum
            ...
            for dist in range(1,11):
                ...
                if j - dist >= 0:
                    sum += grid[i][j - dist]
                if dist >= 4 and j - dist >= 0 and v not in visited:
                    f = g + sum
                    if f < weights.get(v, math.inf):
                        weights[v] = f
                        path[v] = (i, j)
        ...
        if dir == 'h':
            ...
            for dist in range(1,11):
                ...
                if i + dist < len(grid):
                    sum += grid[i + dist][j]
                if dist >= 4 and i + dist < len(grid) and v not in visited:
                    f = g + sum
                    if f < weights.get(v, math.inf):
                        weights[v] = f
                        path[v] = (i, j)
            ...
            for dist in range(1,11):
                ...
                if i - dist >= 0:
                    sum += grid[i - dist][j]
                if dist >= 4 and i - dist >= 0 and v not in visited:
                    f = g + sum
                    if f < weights.get(v, math.inf):
                        weights[v] = f
                        path[v] = (i, j)
